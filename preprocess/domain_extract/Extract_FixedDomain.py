...
import utilities
import configs
from internals import MERRA2_IBTRACS, FNL_IBTRACS, CMIP6_CMIP6TRACKS, ERA5_IBTRACS, MultiProcessing
...
def Fnl():
    ...
    out_dir = CONFIG.OPATH.FIXED_DOMAIN
    utilities.CleanDir(out_dir)
    FixedDomain_Main(FNL, IBTRACS, CONFIG.OPATH.PREPROCESSED, out_dir, configs.nworkers.FNL_FIXEDDOMAIN)
...
def Merra2():
    ...
    out_dir = CONFIG.OPATH.FIXED_DOMAIN
    utilities.CleanDir(out_dir)
    FixedDomain_Main(MERRA2, IBTRACS, CONFIG.OPATH.PREPROCESSED, out_dir, configs.nworkers.MERRA2_FIXEDDOMAIN)
    print("NASA-MERRA2 dataset extraction: Done.")
    return

def Cmip6():
    print("CMIP6 dataset extraction")
    CMIP6, CMIP6TRACS = CMIP6_CMIP6TRACKS()
    out_dir = CONFIG.OPATH.FIXED_DOMAIN
    utilities.CleanDir(out_dir)
    FixedDomain_Main(CMIP6, CMIP6TRACS, CONFIG.OPATH.PREPROCESSED, out_dir, configs.nworkers.MERRA2_FIXEDDOMAIN)      
    print("CMIP6 dataset extraction: Done.")
    return

def Era5_extract():
    print("ERA5 dataset extraction")
    ERA5, IBTRACS = ERA5_IBTRACS()
    out_dir = CONFIG.OPATH.FIXED_DOMAIN
    utilities.CleanDir(out_dir)
    FixedDomain_Main(ERA5, IBTRACS, CONFIG.OPATH.PREPROCESSED, out_dir, configs.nworkers.MERRA2_FIXEDDOMAIN)
...
def Main():
    ...
    parser.add_argument('--dataset', type=str, required=True, choices=['fnl', 'merra2', 'cmip6', 'era5'], help='Dataset to process')
