...
class HurricaneTrack:
    ...
    def SaveToDisk(self, path: str, dataframe: pd.DataFrame) -> int:
        # try:
        #     result = dataframe.to_csv(path_or_buf=path, sep=",", index=False)
        #     print(result)
        #     return 0
        # except:
        #     return -1
        try:
            if dataframe.empty:
                print(f"Warning: dataframe is empty, writing empty CSV to {path}")
            dataframe.to_csv(path, sep=",", index=False)
            print(f"CSV saved to {path}, rows={len(dataframe)}")
            return 0
