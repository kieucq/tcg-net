...
def data_label(args):
    ...
    for step in range(0, 20, 2):
        ...
        for ds, irange in dataset.items():
            ...
            , 'Label'] = 1 # assign positive label (center from 0 to $step)
            
            df.loc[
                (df['Year'].between(*irange, inclusive='both')) &
                (df['Position'] == 0) &
                (df['Step']).between(step, 40, inclusive='right')
            , 'Label'] = 0 # assign negative label (center from $step+1 to 40)
            
            df.loc[
                (df['Year'].between(*irange, inclusive='both')) &
                (df['Position'] > 0) &
                (df['Step']).between(0, 40, inclusive='both')
            , 'Label'] = 0 # assign negative label (negative from 0 to 40)
