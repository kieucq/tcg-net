...
def GetSample(
    ...
    dt_r = [(date_c, time_c)]
    target_lat_dim = int(lat_dim)
    target_lon_dim = int(lon_dim)
    lat_half_dim = int(target_lat_dim/2)
    lon_half_dim = int(target_lon_dim/2)
    ...
    if s_wds is None:
        return None
    if (
        s_wds.sizes["latitude"] != target_lat_dim
        or s_wds.sizes["longitude"] != target_lon_dim
    ):
        return None
