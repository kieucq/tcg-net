...
def plot_dynamic():
    ...
    for name, grp in df.groupby('Datetime'):
        for step in range(0, 20, 2):
            ...
            lon_vals = np.sort(grp['Lon'].unique())
            lat_vals = np.sort(grp['Lat'].unique())
            ...
            if step == 0:
                arr = grp.pivot_table(index='Lat', columns='Lon', values='Score_step2').values
            else:
                arr = grp.pivot_table(index='Lat', columns='Lon', values=f'Score_step{step}').values
            ...
            plt.title(f"{name} - Forecast lead time (step) {step}", fontsize=10)
            plt.tight_layout()
            filename = "".join(str(name).split())
            plt.savefig(f'./output/postprocess/{cycle}/step{step}_{filename}.pdf',
